...
scan_content(){
  ...
  if is_test_path "$f"; then
    ...
    if [ -n "$added" ] && printf '%s\n' "$added" | grep -Eq "$TI_SKIP"; then
      ti_fail=$((ti_fail+1)); ti_files="$ti_files ${f}(skip/focus-added)"
    elif [ -n "$removed" ] && printf '%s\n' "$removed" | grep -Eq "$TI_ASSERT" \
         && { [ -z "$added" ] || ! printf '%s\n' "$added" | grep -Eq "$TI_ASSERT"; }; then
...
while IFS=$'\t' read -r a d p; do
  ...
  if [ "$a" = "-" ] || [ "$d" = "-" ]; then
    ...
    fdiff="$(git diff "$base" -- "$p" 2>/dev/null || true)"
    added="$(printf '%s\n' "$fdiff" | grep '^+' | grep -v '^+++' | sed 's/^+//' || true)"
    removed="$(printf '%s\n' "$fdiff" | grep '^-' | grep -v '^---' | sed 's/^-//' || true)"
