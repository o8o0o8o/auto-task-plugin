...
| `cloudinary_cloud_name` | `""` | Cloudinary cloud name (from the dashboard). Not a secret — it's in every delivery URL. Empty → embedding is inert. |
| `cloudinary_upload_preset` | `""` | An **unsigned** Cloudinary upload preset. Not a secret. Recommend restricting it (allowed formats/size, fixed folder, moderation) — an unsigned preset is world-writable. Empty → embedding is inert. |
...
Set `visual_assets_enabled: true` (off by default; `/auto-task` asks once per repo on UI-scoped runs) and the run also embeds a **before/after screenshot pair** in the PR. Images are uploaded to **Cloudinary** via an **unsigned** upload preset — set `cloudinary_cloud_name` + `cloudinary_upload_preset` (both are public identifiers, not secrets; create an *Unsigned* preset under Cloudinary → Settings → Upload → Upload presets). The delivery URL renders **inline for public and private projects alike** (GitHub proxies external images through its Camo cache). This needs no `gh`, no repo, and no API secret, so it works from any checkout including a fork PR. Two caveats: an unsigned preset is **world-writable** (restrict it — allowed formats/size, fixed folder, moderation), and unsigned upload **cannot delete**, so screenshots **persist** on Cloudinary (the free tier is ample for KB-scale crops). Embedding is best-effort presentation — if it's unconfigured (either key unset) or the upload returns no `secure_url`, the PR just carries a local-artifact + preview note; it never blocks the run.
