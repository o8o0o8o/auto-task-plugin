...
**Where independence actually lives — read this before assuming a phase is a second opinion.** Phases 2, 3 and 4 are **skills invoked in the main loop**, so the same model that wrote the code also self-verifies and code-reviews it. That is deliberate: those skills need the run's full context, and Phase 4 in particular is pinned to the `auto-task-code-review` skill by `enforce-gates.sh` (`gates.code_review.tool`), which is what stops a bespoke review prompt being substituted for a disciplined one. Independent, fresh-context judgment enters at exactly two points — **Gate A** and **Gate B**, each a `task-execution-verifier` Agent spawned with only the diff, the plan and the prior-review history, never the parent conversation. If you want a second model looking at the work, those are the gates that do it; a green Phase 3 or Phase 4 is the author checking their own work, and the pipeline does not claim otherwise. **All Agent spawns are synchronous** (`run_in_background: false`) so the verifier's report is the tool result — a backgrounded spawn returns launch metadata instead, and the run would have nothing to act on.
