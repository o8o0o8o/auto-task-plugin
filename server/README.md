...
| `fix_iterations` / `review_iterations` | number | Loop counts. `review_iterations` counts only the Phase-4 rounds that **reopened** the loop, not every round run — pair it with `review_rounds` below. |
| `review_rounds` | number \| null | **Added in [v7].** Phase-4 review rounds **run**, reopening or not. Read against `review_iterations`: rounds rising while reopenings hold flat means the review is running more and finding no more. **Purely additive — no existing field changed** in v7, so a v6 query stays correct verbatim. **`0` is a measurement ("ran no rounds"); `NULL` means the row cannot say — and NULL is not a pre-v7 marker.** Two kinds of row carry it: every pre-v7 row, which never had the field, *and* a v7 row whose sender state predates `rounds[]` and so could not be measured either. `schema_version < 7` is therefore **strictly narrower than** `review_rounds IS NULL` and must not be used as a stand-in for it. **Exclude NULLs; never coalesce them to 0** — that rule is correct on its own and needs no version test. Coalescing would report a review-volume drop that never happened. |
| `gate_b` | string | `"passed"` or a skip reason. |
| `followups` | number | Parked follow-up count. |
| `duration_min` | number \| null | Wall-clock. **Semantics AND nullability changed in [v6]:** from v6 it is **measured** from the sender's hook-stamped run clock (`date -u` at run start and last activity), and is `null` when the span was **rejected** as implausible (negative, or over 12h — typically a run paused overnight). **It is NOT null merely because nothing was measured:** when the sender has no clock the field falls back to the pre-v6 history-timestamp derivation, which still yields `0` if that history is unusable. So a v6 `0` can be either a real zero-minute run or an unmeasured one, exactly as in v5 — `schema_version` tells you the *rejected* case is now expressible as NULL, not that every unmeasured duration is. Treat `0` as suspect in every version. Through v5 it was derived from model-written history timestamps (the model has no clock, so those values were guesses) and was **always a number**, falling back to `0`. The two are indistinguishable by value — a `0` reads as a real fast run in every version — so branch on `schema_version` before treating any duration as measured, and expect `NULL` from v6 **for rejected spans specifically**. |
...
| `act_duration_min` / `act_tokens` | number \| null | Measured actuals. `act_tokens` is the grand total (input+output+cache_read+cache_creation) and is cache_read-dominated, so it is recorded but is not the est-ratio counterpart; unchanged in v5 and v6. **`act_duration_min` changed in [v6]** on the same two axes as `duration_min` — from v6 it is clock-derived and is `null` on a rejected span, and it no longer mirrors the sender's `actuals.duration_min` even when that holds a number (the clock wins). Branch on `schema_version` for this field too, not only for `duration_min`. |
...
A `[vN]` marker normally means **field presence** from version N. Three fields are exceptions — `est_tokens`, `duration_min` and `act_duration_min` are present in every version, and their `[v5]` / `[v6]` markers flag a **semantics** change (for the two duration fields, a nullability change too). `est_tokens` held a cache-inclusive token *total* through v4 and holds the predicted *output* tokens from v5; the duration fields were derived from model-written timestamps and always numeric through v5, and are clock-measured and nullable from v6. The distinction matters because a presence-marker can be handled by a `NULL` check, whereas a semantics change cannot: for `est_tokens` the two scales are ~100× apart, and for the durations a v5 fallback `0` is indistinguishable by value from a real fast run. A query pooling rows across those boundaries must branch on `schema_version` before computing any est/act ratio.
