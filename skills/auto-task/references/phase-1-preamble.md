...
_Split out of `skills/auto-task/SKILL.md`; every section titled `X — relocated verbatim from SKILL.md` is verbatim. Two content sections are NOT relocations — `user-facing-voice` and `plan-approval-presentation`, both added by the plain-language pass; each says so in its own opening line. (`## Cross-references` below is navigational scaffolding, present in every reference file, and is not relocated content either.) `SKILL.md` points here with a MANDATORY READ directive._
...
## user-facing-voice — the plain-language rule

_New in the plain-language pass, not relocated. Binding for every string a **human** reads: `AskUserQuestion` questions and option labels, gate prompts, pause messages, the surfacing status, and the plan presentation. NOT binding for text a **model** reads (hook stderr, spec prose, agent instructions, `state.history`/TRACE entries) — that stays precise and directive, and shortening it would weaken the guarantees it enforces. This sits beside `comment-voice` below: that one governs prose OTHER people read on a ticket or PR, this one governs what the user in front of the run reads._

A user meets these strings mid-task, usually while blocked on a decision. They have not read this spec. They should be able to answer without looking anything up.

1. **Consequence before mechanism.** Say what happens if they pick this, then how it works — never the reverse. Not "uploads via an unsigned preset and embeds inline" → "your screenshots become publicly viewable."
2. **No unexplained jargon.** Any term that exists only inside this plugin — Gate B, blast radius, AC, tier, INCONCLUSIVE, `expected_next_action`, fail-open, loop-rule clause N — is replaced with ordinary words or given a half-sentence gloss on first use. Ordinary engineering words (branch, commit, PR, tests) need no gloss.
3. **One question per question.** If a prompt asks two things, it is two questions. Never bundle a second decision into a description.
4. **Short sentences.** Aim under 25 words. Break a long one rather than adding another em-dash clause.
5. **Options are answers, not categories.** A label reads as the thing the user is choosing to do — "Push and open a PR", not "PR mode". Under ~6 words.
6. **Lead with the recommendation**, marked `(Recommended)` — what you would pick alone. The exception is a consent prompt whose safe default is "no": there, decline goes first, because the default must be the easy answer.
7. **Say the default and the escape hatch once.** "Off unless you turn it on. You can change this later." One sentence, not a paragraph.
8. **Pointers are asides, never the answer.** "see README → Remote telemetry" may trail a prompt; it may not carry information the user needs in order to decide.
9. **Never lose a constraint to brevity.** Anything that limits what the user is agreeing to — what leaves their machine, what becomes public, what cannot be undone — stays in the prompt, in plain words. Cutting it is a bug, not a simplification.
10. **Headers are chips, not sentences.** ≤12 chars, naming the decision: `Telemetry`, `Screenshots`, `Push`.

**The test before shipping a prompt:** read it to someone who has never used this plugin. If they ask "what does that mean?" or "what happens if I say yes?", it fails.
...
3. **Decide:** if `$out` is **non-empty** (a newer version exists), ask ONCE via `AskUserQuestion`: present `$out` and ask *"A newer version of auto-task is available. Update before starting?"* with two options — **"No, start the run now (Recommended)"** (description: "Uses the version you have. You can update any time.") and **"Yes, update it for me"** (description: "Installs the new version. You'll need to restart Claude Code, then run `/auto-task` again."). Header chip: `Update`. On *proceed* → continue into Component preflight. On *update* → **auto-apply the update, no manual command**:
...
3. **Ask exactly once** via a single `AskUserQuestion` (part of the Phase-1 human surface; `approved` is still `false`, so the Stop hook allows the yield — set `expected_next_action` `null`/`"user-approval"` as for any Phase-1 ask): *"Send anonymous usage stats for this repo? Mostly numbers: how long runs take, how often they finish. Also your OS and version numbers (Claude, Claude Code, this plugin), plus a random ID for this machine you can reset or delete. Your code, task descriptions, branch names, and file paths never leave your machine. One exception: if you type a note at the end of a run, that note is sent word for word. Off unless you turn it on; you can change it later."* Options: **"No thanks"** (description: "Nothing is sent. I won't ask again for this repo.") / **"Yes, send anonymous stats"** (description: "Helps improve auto-task. Never your code or task text."). Header chip: `Usage stats`. (Full detail lives in README → Remote telemetry, but the prompt must stand on its own.)
...
4. **Ask exactly once** via a single `AskUserQuestion` (Phase-1 surface; set `expected_next_action: "user-approval"`): *"Show before/after screenshots in the PR? To do that, auto-task uploads the screenshots to an image host, so anyone with the link can view them — even if your repo is private. Off unless you turn it on; you can change it later."* Options: **"No thanks"** (description: "No screenshots are uploaded. I won't ask again for this repo.") / **"Yes, show screenshots in PRs"** (description: "Uploaded images are publicly viewable by link. Uses a shared image host by default, or point the `cloudinary_*` settings at your own account."). Header chip: `Screenshots`. Surface with the run banner.
...
3. **Create the per-branch folder.** `mkdir -p .auto-task/<branch>/artifacts .auto-task/<branch>/recon .auto-task/<branch>/fixes`. Slashes in the branch name are preserved literally (branch `fix/auth-bug` → `.auto-task/fix/auth-bug/`). This MUST match `git branch --show-current` verbatim — the gate and Stop hooks resolve `.auto-task/<branch>/STATE.json` from it, and any divergence (extra prefix, rewritten slug) makes them silently find no state file and fail open.
   ...
   **Then pin the untracked baseline: `git ls-files --others --exclude-standard -z > .auto-task/<branch>/untracked-baseline`.** This records the paths that were ALREADY untracked before the run touched anything, and `hooks/intent-add-untracked.sh` (Phase-3 entry) skips every path in it, so the run's sweep intent-adds ONLY files the run itself created. On the normal path the file is **empty** — a fresh worktree cut from the default branch is clean — so this is a no-op for almost every run. It is load-bearing on the two paths that carry a user's WIP into the run's tree (the already-in-a-worktree path in step 1, and the in-place fallback in step 3), where the "Pre-existing staged/unstaged changes" rule below forbids `git add` on that WIP: intent-adding a user's untracked file makes an ordinary `git restore .` / `git checkout -- .` truncate it to zero bytes and a hard reset remove it entirely, which is why the exclusion is pinned here rather than left to a later cleanup. Write it with `-z` (NUL-delimited) so any path git can produce round-trips. Pin it **before** any other run activity, and never refresh it mid-run — a path that becomes untracked later is the run's, not the user's.
...
4. **Route, then ask (the two-step clarify surface).** If the Asked bucket is empty after stages 1–3, skip this step entirely — do NOT invent questions to "look thorough" (no router, no comment, nothing to record). A run where every ambiguity was resolved with evidence is a *better* run, not a lazier one. Otherwise, present the Asked bucket through the routing flow below.
   ...
   **Step 4a — routing question (ALWAYS FIRST, MANDATORY).** This step runs only when the Asked bucket is non-empty and `clarify_forward_pending.pending` is false (on resume from a forwarded pause, the Resume dispatcher at the top of this gate has already handled it before this point). Before showing any clarifying question, present a single routing `AskUserQuestion` — "I have <N> question(s) before I can plan this. How do you want to handle them?" (header chip: `Questions`) — with exactly two options:
   - **`Answer them here`** — description: "I'll ask them now, one at a time." The user answers inline.
   - **`Give me a comment to forward`** — description: "I'll write the questions up so you can paste them to whoever owns this ticket. The run pauses until you're back with answers." Render a paste-ready ticket comment.
   ...
   **Step 4b-answer — the user chose `Answer them here`.** Present the Asked-bucket questions via `AskUserQuestion` (1–4 per call; if more than 4, prioritize the highest-impact and fold the rest into PLAN.md's Unknowns). Each question MUST:
   ...
   - **Obey the `user-facing-voice` rule** (this file, above the `comment-voice` section) — plain words, no plugin jargon, one decision per question, options phrased as answers rather than categories. A user should be able to answer without reading the plan first.
   ...
   **Step 4b-forward — the user chose `Give me a comment to forward`.** Render a **paste-ready ticket comment** wrapping the Asked-bucket questions (style contract below) in a fenced block under a short lead-in like "Forward these to the ticket owner:". Then PAUSE the run so the user can get the answers from the owner:
   ...
   - Set `expected_next_action: "user-approval"` (a legitimate Phase-1 pause while `approved:false` — the Stop hook allows it) and STOP with a short banner'd surface: "Paste this into the ticket. Once you have the answers, run `/auto-task` again (or just paste them here) and I'll pick up where I left off."
...
2. **Pin the baseline** — write the output (or a summary if large) to `.auto-task/<branch>/recon/ac-<#>-baseline.{json|txt}` and reference it from `state.history` as `{ phase: "define-preflight", ac: <#>, polarity: "change|invariant", result: "pinned|vacuous|pre-broken|failed-syntax|unreliable-signal", baseline: "<value or path>", at: "ISO-8601" }`.
3. **Discrimination check — the baseline must be able to move.** A pinned baseline is only evidence if the check can tell "done" from "not done". Classify each AC's **polarity** from its own text and record it in the entry above:
   - **`change`** — the criterion asserts something this run will MAKE true (a new test passes, a new export exists, an endpoint returns 200, a deprecated-string count reaches 0). The baseline MUST **fail**; that failing baseline IS the AC's proof of discrimination.
   - **`invariant`** — the criterion asserts something that must STAY true (the existing suite stays green, the build keeps succeeding, typecheck stays clean, no NEW lint errors). The baseline MUST **pass**; that passing baseline is the thing being preserved.

   **Neither cell fits?** An AC that asserts BOTH a new behaviour and a preserved one is two criteria wearing one row — **split it** and classify each half, since a single result value cannot carry both directions. An AC whose check has no meaningful pre-change signal at all is not a `change`/`invariant` question: it has no executable command to dry-run, so this step never applied to it (its `Gate` is `gate-a` per rule 6, not `self-verify`). Those two cases are exhaustive; do not invent a third polarity value.

   Compare each pinned baseline against its polarity. Two cells are defects, and they route differently because one is a flaw in the AC and the other is a flaw in the premise:
   - **`change` + baseline already passes → `vacuous`.** The check cannot witness the work: it reports success before the work exists, so a Phase-3 pass on it would prove nothing and Gate A would have no movement to compare. This is a defect in the **AC**, not in the repo — **rewrite the AC into a check that discriminates** (name the specific new test rather than the whole suite; assert the new field rather than that the call merely returns 200) and re-run pre-flight for that AC, exactly as with `failed-syntax`. Do NOT stop for human approval. Bounded by the **rewrite budget** below, which it shares with `failed-syntax`.
   - **`invariant` + baseline already fails → `pre-broken`.** The invariant is already violated at `state.base`, so it cannot serve as this run's regression guard, and the run must not adopt a red check it did not cause — left in place it spends fix-loop rounds on someone else's breakage and can converge the loop on a defect outside the task. **STOP and surface BEFORE the human gate** with the failing output, exactly like the FP > 20% path: the user decides whether to fix the pre-existing breakage first, narrow the AC to the currently-green subset, or drop it.

   **Why this step exists.** A check written while the implementation is in view tends to be one the implementation already satisfies — the assertion takes its expected value from the observed behaviour instead of from the criterion. A check that has never been observed to fail is indistinguishable from a check that *cannot* fail, and that failure is silent: every gate reports green and nothing was verified. Pinning the failing baseline is what makes a later pass mean something — the property benchmarks call **fail-to-pass** — and pre-flight is the only moment the pre-change tree still exists to establish it.
4. **Sample-verify when the AC depends on an external tool's output** (knip, jscpd, ts-prune, knip-ish dead-code detectors, complexity scanners, dependency analyzers, anything that produces a list of "things to fix"):
...
5. **Pre-flight syntax check.** If the AC command itself errors out (jq path wrong, file not found, tool not installed) — fix the AC command, not just the symptom. An AC that can't be executed at all is also unreliable. Bounded by the same **rewrite budget** as `vacuous`.

**Rewrite budget (applies to `failed-syntax` and `vacuous` alike).** Both routes fix an AC and re-run pre-flight without stopping for approval, so both can spin. **At most two rewrites per AC**, counted as the `define-preflight` entries already logged for it — log every attempt, so the count is reconstructible on resume rather than held in the model's head. If the third dry-run still errors, or still reports success before any work exists, stop rewriting: the criterion is not expressible as a check that runs and discriminates, which is the "cannot articulate measurable AC" case in the Acceptance Criteria contract — STOP and surface, naming the attempts. These are the two routes in Phase 1 that loop without a human in the way, and no hook watches them (`enforce-gates.sh` sees only `git commit`, which Phase 1 never reaches), so the bound has to live in the contract — the same reasoning that bounds Gate B and the fix loop.

Pre-flight produces one of five outcomes:

- **Every AC pinned with a baseline consistent with its polarity, FP ≤ 20% on every sampled list** → advance to the critique pass.
- **Any AC's command errors (`failed-syntax`)** → fix the AC text (re-write the command), re-run pre-flight for that AC. Do not stop for human approval until every AC has a clean dry-run — within the rewrite budget above.
- **Any `change`-polarity AC whose baseline already passes (`vacuous`)** → rewrite the AC into a discriminating check and re-run pre-flight for it. Do not stop for human approval; after **two** failed rewrites (the shared rewrite budget), surface as an unmeasurable criterion.
- **Any `invariant`-polarity AC whose baseline already fails (`pre-broken`)** → STOP and surface BEFORE the human gate. The run must not inherit a pre-existing failure as one of its own criteria.
- **Any sampled list shows FP > 20% (`unreliable-signal`)** → STOP and surface BEFORE the human gate. The plan is built on a wrong premise; user must pivot scope or switch tools.

Pre-flight evidence (the pinned baselines + polarity + sample-verification log) MUST appear in `.auto-task/<branch>/PLAN.md` as a `## AC Pre-flight` section between Acceptance Criteria and Critique, with one terse bullet per AC: `AC #N — <change|invariant>; baseline pinned (<value>), <fails|passes> as required; sample-verified N entries, FP=X%`.
...
## plan-approval-presentation — how to present the plan at the gate

_New in the plain-language pass, not relocated. The spine's Phase-1 approval step ("present the plan summary … and STOP") owns the CONTRACT — what must be visible (Critique, risk disclaimer, `## Clarifications`, the Decision watchlist, the Estimate) and the approval keywords. This section owns only the WORDING, under the `user-facing-voice` rule above. It adds no gate, no yield, and never alters `expected_next_action`._

This is the most-read surface in a run, and often the user's first sight of it. Lead with two or three sentences of plain prose — what you are going to change, and the one thing most worth a second look — before any table.

Keep the internal vocabulary out of the narration: "how big and risky this looks" rather than "tier / D-R scores"; "the parts of the codebase this touches" rather than "blast radius"; "how I'll prove each one works" rather than "verification method"; "I couldn't check this one without a running app" rather than "INCONCLUSIVE".

**The tables stay structural.** The Acceptance Criteria table, the task breakdown, and the Estimate keep their column names and machine-readable content verbatim — this rule governs the prose around them, never the tables, checklists, headings, or diagrams. Nothing that must be visible per the spine's contract may be dropped for readability; rule 9 applies here exactly as it does to a prompt.

Close with a plain line saying what to type to start, and that they can ask for changes first.
