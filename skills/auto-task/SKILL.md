...
**Settings + preview + bot_review objects (`settings`, `preview`, `bot_review`).** All additive. `settings` is the Phase-1 snapshot of the resolved project settings (see "User settings" below) — a record of what the run read, not a live source; it is invisible to every hook. `preview` is populated only by Phase 7 (preview verification); `bot_review` only by Phase 6 (bot-comment review). Both are `null` on every run where their phase does not execute (the default, since `has_preview_deployment` defaults to `false` and `bot_review_autofix` defaults to `false`). `enforce-gates.sh` and `record-outcome.sh` read neither object (they key on `gates.*` and `phase` respectively, and both handle any phase string generically). The **only** hook that reads these objects is the Stop hook (`prevent-mid-protocol-stall.sh`): its soft-lock signature includes `preview.polls` AND `bot_review.polls` so the `poll` waits of Phase 7 and Phase 6 — legitimately long-lived `auto-continue` states — are recognized as *progressing* each time the respective poll counter bumps, instead of being misread as a frozen run. That signature line is the ONE hook edit this feature required; all other hooks are phase-agnostic and untouched.
