...
| `budget_blowout_factor` | `3` | Soft interrupt: when measured tokens/time exceed the Phase-1 estimate by this factor, the run checks in (loop-rule-family, model-driven — there is no token-usage hook, so this one is not hook-enforced). |
...
- **Budget blowout** — a *soft* interrupt (loop-rule-family, model-driven; there is no token-usage hook): when measured cost exceeds the estimate by `budget_blowout_factor`, check in with the user. This one is deliberately not hook-enforced and is labeled as such rather than overclaimed.
...
The floor is prose the model must honor (no hook enforces AC pass/fail — hooks gate only `gates.*` booleans + the review-staleness hash). Its teeth are: (a) the "cannot set the gate unless every AC recorded pass" rule above, (b) the Gate A verifier being explicitly told to reject a proxy standing in for a live-declared method, and (c) this being a visible, reviewable contract so a false PASS is a catchable violation rather than an invisible judgment call.
...
12. Write `pr_url` to state, then **decide the terminal transition**. Two post-PR phases can run, **in order: Phase 6 (bot-comment review) → Phase 7 (preview verification)**. Both require that a push actually happened (the user chose push/push-and-PR at step 8, not "hold"). Evaluate in this order:
...
**6. Terminal handoff.** After bot-review completes, decide the next transition:
...
**1. Resolve the preview URL (bound to the pushed commit).** `HEAD_SHA = git rev-parse HEAD`.
   ...
   4. **No URL by `timeout_min`.** If NONE of the sources yields a reachable URL in the bound: in the **explicit** case (`has_preview_deployment` true) record `preview.status = "pending"` and surface (`user-approval`) to resume later. In the **auto-learn** case (`has_preview_deployment` was unset), do NOT nag and do NOT persist a negative: leave `has_preview_deployment` unset, record `preview = { status: "skipped-learned-none", learned: null, verdict: null, at }`, log `{ phase: "phase-7-preview-autolearn", learned: null, at }` to `state.history`, and then **apply the external-actions terminal guard below** (a declared external action still gates completion even though the preview was skipped) — only if there are no declared external actions do you set `phase: "done"`, `expected_next_action: null` and finish with the normal final summary. Because nothing is persisted, a later post-PR run re-attempts detection — a slow deploy bot or a degraded check (no `gh`/auth/PR) is thus a transient miss, never a permanent `false`; the user sets an explicit `false` to stop the per-run re-check. A repo without a deploy comment must feel exactly like a repo with preview disabled — no failure, no stall, no extra prompt. Never emit FAIL for a missing preview (no preview ≠ a regression).

   **External-actions terminal guard (applies to EVERY Phase-7 exit that would otherwise set `phase: "done"` — the `skipped-learned-none` auto-learn case above, and the PASS/FAIL/INCONCLUSIVE verdict terminal in step 4).** Before writing `phase: "done"`, check the run's declared external actions: if any is not yet `applied-verified` / accepted `applied-unverified` / descoped, do NOT go to `done` — set `phase: "external"`, `expected_next_action: "auto-continue"`, and advance to **Phase 8** (the preview outcome is still recorded + reported; Phase 8 owns the terminal state). Only when there are no un-applied external actions does a Phase-7 exit reach `done`. This is the same "external actions gate completion" rule step-12 enforces at the Phase-5 → post-PR fork; it MUST be re-checked at every Phase-7 done-writer so a no-preview / skipped-learned-none run can never short-circuit past a declared external change (the "shipping the script ≠ done" invariant).
