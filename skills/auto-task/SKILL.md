...
**Non-negotiables restated inline:**
- **A finding costs a round only if it (a) breaks an approved AC, (b) is a runtime-reachable regression/bypass, or (c) is a security/data-loss path** — the SAME test Gate B applies, so they can't drift. **YOU grade it** — `auto-task-code-review` emits no `ac:`/`reachable:` field, so classify from PLAN.md's AC table and the diff, and **fail closed on your own uncertainty:** a finding you cannot place counts as AC-breaking. The label never decides control flow.
