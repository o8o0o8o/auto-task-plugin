...
| `visual_assets_enabled` | `false` | Opt-in master switch for **visual-proof embedding** (before/after screenshots pushed to a dedicated GitHub assets repo and embedded in the PR — see Phase 5 "Visual changes"). **Off by default**, per-project; auto-task **asks once per repo** (Phase-1 consent, only when the run has UI/visual scope) before it will push images anywhere. When off, verification still runs locally; the PR just gets local-artifact + preview-link notes instead of embedded images. Enabling it grants the pipeline authority to create/push to a GitHub repo of screenshots — for a **private** project pushed to a **public** assets repo that makes the screenshots world-readable (see `visual_assets_visibility`). Settable per-project or globally (see "Two scopes"). |
| `visual_assets_repo` | `""` | The `owner/repo` the before/after PNGs are pushed to. Empty → **auto-derive** `<pr-repo-owner>/auto-task-assets` and auto-create it (matching `visual_assets_visibility`) if missing. Assets are stored under `<pr-repo>/<branch>/<before\|after>.png`; the run keeps shots tight (element crops) and prunes them when the PR merges/closes. |
| `visual_assets_visibility` | `"public"` | Visibility of the assets repo (and how images are embedded). `public` (default) → repo is public, before/after render **inline** via `raw.githubusercontent.com` URLs. `private` → repo is private, PR embeds **member-only blob-view links** (`github.com/<owner>/<repo>/blob/…`) + a one-line note (raw URLs from a private repo are token-gated and won't render inline). `auto` → **match the project repo's visibility** (public project → public/inline; private project → private/links), so screenshots are never made more public than the project. |
...
**Visual-assets consent check (Phase 1, opt-in — off by default, per-project).** Embedding before/after screenshots in a PR means **pushing images to a GitHub repo** (see Phase 5 "Visual changes"), so — exactly like telemetry and `bot_review_autofix` — it is **off by default** and requires an **explicit per-repo opt-in**. This check is model-driven (a hook can't prompt), best-effort/fail-open, and **skipped on resume**. Unlike the telemetry check it is **UI-scoped**: only ask when this run would actually produce visual proof, so backend/docs/config repos are never nagged.
...
4. **Ask exactly once** via a single `AskUserQuestion` (Phase-1 surface; set `expected_next_action: "user-approval"`): *"Embed before/after screenshots in PRs for visual changes? auto-task would push UI screenshots to a GitHub repo (`<owner>/auto-task-assets`, created if missing) and embed them. Public by default so they render inline — on a **private** project that makes the screenshots public unless you set `visual_assets_visibility: private`. Off by default; change anytime in settings."* Options: **"Enable — embed screenshots in PRs"** / **"No thanks — don't ask again for this repo"**. Surface with the run banner.
...
2. **Build the change diagram.** Produce a human-readable Mermaid diagram that the reviewer can scan in ~10 seconds to understand what the run did and (where applicable) what the prior state looked like. Embed it in the PR body under `## Change diagram` so GitHub renders it inline. This is a MANDATORY step — every auto-task PR must carry a diagram or a one-line documented skip.
   ...
   **Visual changes (embedded before/after — OPT-IN, replaces Mermaid for visual concerns when enabled).** When the change is visual (the diagram-type table's "Visual / UI / CSS change" row) AND visual embedding is enabled, the PR carries a rendered **before/after pair** under a `## Visual changes` section instead of a Mermaid diagram for that concern. (A PR with an additional structural concern still gets that concern's Mermaid diagram; the two coexist. A *pure* visual change gets screenshots and no Mermaid.) **Embedding is best-effort presentation, NOT verification** — the "does it work" judgment already happened in Phase 3 / Gate A on local dev (INCONCLUSIVE if it couldn't). So **nothing in this step ever blocks the run**: every failure path falls back to a note + local artifacts and the run continues.
   ...
   2. **Push to the assets repo, then embed (visibility-aware).** GitHub has no `gh`/REST attachment-upload endpoint, so images are hosted in a **dedicated GitHub repo** the pipeline pushes to with ordinary `gh`/git authority (no browser session needed).
      - **Resolve the repo.** `assets_repo = settings.get visual_assets_repo`; if empty, auto-derive `<pr-repo-owner>/auto-task-assets`. If it doesn't exist, create it with `gh repo create "<assets_repo>" --<public|private>` at the resolved visibility (below). Resolve its **default branch** via `gh` (`gh api repos/<assets_repo> -q .default_branch`) — do NOT hardcode `main`.
      - **Resolve visibility.** `vis = settings.get visual_assets_visibility` (`public` default | `private` | `auto`). `auto` → match the PR repo's visibility (`gh repo view --json isPrivate`). Create/expect the assets repo at that visibility.
      - **Push the PNGs** to `<pr-repo>/<branch>/<before|after>.png` on the assets repo's default branch (a shallow clone + commit + push, or `gh api` blob/commit). Keep the shots tight; a later prune (on PR merge/close) keeps the repo small.
      - **Embed by visibility.** **Public** assets repo → inline images with `raw.githubusercontent.com/<assets_repo>/<default-branch>/<pr-repo>/<branch>/<before|after>.png`. **Private** → the raw URL is token-gated and won't render, so embed **member-only blob-view links** `https://github.com/<assets_repo>/blob/<default-branch>/<pr-repo>/<branch>/<before|after>.png` with a one-line note ("screenshots in the private `<assets_repo>` — visible to repo members").
      - **Failure modes — fall back to a note, NEVER a STOP.** If the push can't happen — **fork PR / no push authority to the assets owner**, **org repo-create denied**, an **existing assets repo whose visibility conflicts** with the resolved setting, or any `gh`/git error — do not retry-block and do not surface as a loop-rule blocker: write the step-0 local-artifacts/preview note (naming the reason, e.g. "couldn't push to `<assets_repo>`: no permission — screenshots are in artifacts/"), log `{ phase: "handover-visual", result: "embed-failed", reason, at }`, and continue to commit the run. The core work already shipped; embedding is presentation.

   3. **Persist the URLs.** On success, save the embedded URLs (raw or blob) to `.auto-task/<branch>/recon/visual-changes.json` (`{ "before": "<url>", "after": "<url>", "kind": "inline|link", "state": "<what UI state was captured>" }`) so the PR-body step and any resume embed them without re-pushing. Log `{ phase: "handover-visual", result: "embedded", kind, before, after, at }`.
...
10. Create the PR with `gh pr create`. Title from the plan summary (under 70 chars). Body:
   ...
   ## Visual changes
   <ONLY for UI-visual changes (else omit this heading entirely). Content depends on `visual_assets_enabled` + `visual_assets_visibility` (from recon/visual-changes.json):
   - embedding on + PUBLIC assets repo → inline before/after table using the raw.githubusercontent URLs: `| Before | After |` / `|---|---|` / `| ![before](<before-url>) | ![after](<after-url>) |`
   - embedding on + PRIVATE assets repo → member-only blob-view links (not inline images): `Before: <blob-url> · After: <blob-url>` + note "visible to repo members"
   - embedding off, or embed failed → the one-line note from the change-diagram step (screenshots in local artifacts; preview link if any)
