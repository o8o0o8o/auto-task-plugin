...
Phase 1 contains the **only** human-interaction surface in the entire pipeline. It has two parts that the user sees: (1) a single batch of clarifying questions presented up-front via `AskUserQuestion`, and (2) plan approval at the end of Phase 1. Both happen inside Phase 1 — there is no second human gate later. **Part (2) is gated by the `require_plan_approval` repo setting (default `true` = today's behavior):** when it is explicitly `false` AND the run did not trigger the mandatory risk disclaimer, plan approval is auto-satisfied (no wait — see Phase 1's approval step) and the pipeline flows straight into Phase 2. Part (1) is unaffected — a genuine unresolvable ambiguity still surfaces even when the gate is off. A high-risk run always pauses for approval regardless of the setting.

After the user approves at the Phase 1 plan gate (typing `approved` / `looks good` / `proceed` / `yes` / `go ahead`), OR the plan is auto-approved because the gate is disabled and the run is not high-risk, the pipeline runs to one of two terminal states **without stopping for the user**:

- **Success:** Phase 5 completes (commit landed + PR open OR explicit user choice to hold the push at the final handover prompt) — and, when a push happened, the post-PR phases run in order: **Phase 6 (bot-comment review, opt-in)** conservatively applies any actionable Cursor/GitHub-bot PR-comment fixes, then **Phase 7 (preview verification, when enabled/auto-learned)** records a final verdict (PASS/FAIL/INCONCLUSIVE = terminal `done`). A preview `handoff` or timeout (`pending`), and a Phase-6 poll/surface, are legitimate post-PR surfaces per the yield-point table (work still owed), not mid-pipeline stalls.
...
**Settings + preview + bot_review objects (`settings`, `preview`, `bot_review`).** All additive. `settings` is the Phase-1 snapshot of the resolved project settings (see "User settings" below) — a record of what the run read, not a live source; it is invisible to every hook. `preview` is populated only by Phase 7 (preview verification); `bot_review` only by Phase 6 (bot-comment review). Both are `null` on every run where their phase does not execute (the default, since `has_preview_deployment` defaults to `false` and `bot_review_autofix` defaults to `false`). `enforce-gates.sh` and `record-outcome.sh` read neither object (they key on `gates.*` and `phase` respectively, and both handle any phase string generically). Two Stop hooks read these objects. `prevent-mid-protocol-stall.sh`: its soft-lock signature includes `preview.polls` AND `bot_review.polls` so the `poll` waits of Phase 7 and Phase 6 — legitimately long-lived `auto-continue` states — are recognized as *progressing* each time the respective poll counter bumps, instead of being misread as a frozen run. And `send-telemetry.sh`: at `phase==done` it reads `.preview.verdict` into the anonymized payload (see "Remote telemetry" below). No other hook reads these objects — `enforce-gates.sh` / `record-outcome.sh` and the rest are phase-agnostic; the anti-stall signature line is the one edit the preview/bot-review polls required.

**Terminal vs in-flight for `phase: "bot-review"` / `phase: "preview"`.** The hooks treat any non-`"done"` phase as an active run, and `record-outcome.sh` fires only at `phase: "done"`. Phases 6 and 7 therefore map their outcomes so the ledger/telemetry stay honest. **Phase 7:** a **completed** verification — verdict `PASS`, `FAIL`, or `INCONCLUSIVE` — is TERMINAL (`phase: "done"`), so it is recorded and never lingers as "stalled"; an **incomplete** verification — `awaiting` (handoff, deferred) or `pending` (deploy not ready by timeout) — stays `phase: "preview"` and is correctly counted as in-flight until a resume finishes it. A `FAIL` is done-with-a-negative-verdict (fixing the regression is a *new* run), not an open loop. **Phase 6:** a completed bot-review (`applied` / `no-action` / `skipped-*`) hands off to Phase 7 or to terminal `done`; it only stays `phase: "bot-review"` while genuinely in-flight (polling, or a surfaced blocker awaiting the user).
...
| `"user-approval"` | A legitimate human gate. Stop hook allows. | Phase 1 plan-presentation point (before user types `approved` / `proceed`) — **when `require_plan_approval` is on, OR the run is high-risk** (a disabled gate does not skip the disclaimer stop). The one-time opt-out ask after a first manual approval. Loop-rule surface (when the Surfacing protocol fires). |
...
| Phase 1 plan presented for approval (gate ON, or gate OFF + high-risk disclaimer fired) | `"user-approval"` |
| Phase 1 plan auto-approved (gate OFF, i.e. `require_plan_approval === false`, AND not high-risk) | `"auto-continue"` (and `approved: true`, `phase: "execute"` — no yield, continue straight to Phase 2) |
| Phase 1 one-time opt-out ask (fires after a first *manual* approval when `require_plan_approval` was never explicitly set) | `"user-approval"` — issued BEFORE the `approved: true`/`auto-continue` write, so the Stop hook allows it |
...
| Phase 5 push/PR done, no post-PR phase applicable (`bot_review_autofix` false AND preview not applicable, or push held) | `null` (and `phase: "done"`) |
...
| Phase 6 completes → no preview applicable | `null` (and `phase: "done"`) |
...
| Phase 7 verdict PASS / FAIL / INCONCLUSIVE, state written | `null` (and `phase: "done"`) — a completed verification is terminal; FAIL is reported prominently but the run is done (fix = new run) |
...
| `require_plan_approval` | `true` | Whether Phase 1 **waits** for explicit human plan approval. Default `true` = today's behavior (present plan → stop → wait). Explicit `false` = auto-approve routine plans and run fully unattended from intake. **Two guardrails hold even when `false`:** (a) a run that triggers the mandatory **risk disclaimer** (HEAVY tier / irreversible / auth-payments) still pauses for approval; (b) **clarifying questions** still surface for genuine unresolvable ambiguity — disabling removes only the approval *wait*, nothing else. On the **first manual approval** in a repo where this was never explicitly set, Phase 1 asks once whether to disable the gate and persists the answer (project-scoped), so it is never asked again; dismissing that ask leaves the gate on. Settable per-project or globally (see "Two scopes"). |
...
4. **Ask only the Asked bucket** via `AskUserQuestion`. Present 1–4 questions per call (the tool's cap). If you genuinely have more than 4 items in the Asked bucket, prioritize the highest-impact and fold the rest into PLAN.md's Unknowns. Each question MUST:
   ...
   <!-- TICKET-COMMENT-BEGIN -->
   **Forward to the ticket owner (render alongside the `AskUserQuestion`).** The person running `/auto-task` is often not the person who owns the ticket and holds the answers. So whenever the Asked bucket is non-empty and you present it via `AskUserQuestion`, IN THE SAME MESSAGE (right below the prompt) ALSO render a **paste-ready ticket comment** wrapping those same questions, so the user can forward them to the actual ticket owner instead of answering themselves. This is strictly an **alternative, not a replacement**: the user may *answer here* inline via the `AskUserQuestion`, OR *copy the comment* into the ticket — offering both adds **no new prompt, no new yield, and no new `AskUserQuestion`** (it is plain message text beside the existing single call, so `expected_next_action` is unchanged).

   Render it in a fenced block (so it copies cleanly) under a short lead-in like "Or forward these to the ticket owner:". The comment MUST follow this style contract:
   ...
   **Record it.** When you write the `## Clarifications` section in stage 5 below, include the rendered comment verbatim in its `### Ticket comment` subsection (defined in that stage's template), so it is persisted with the questions it wraps (and available on resume / to a later reviewer). If the Asked bucket is empty, there is no comment to render or record.
   <!-- TICKET-COMMENT-END -->

5. **Record everything.** Write a `## Clarifications` section at the very top of `.auto-task/<branch>/PLAN.md` (above Feasibility). The section contains the two buckets, followed by the Decision watchlist view (a subset of Resolved — see the promotion rule below), and finally the paste-ready Ticket comment (only when the Asked bucket is non-empty — see stage 4), in this order:
   ...
   ### Ticket comment (paste-ready, only when the Asked bucket is non-empty)
   <the exact human-like, name-free, business-only comment rendered alongside the AskUserQuestion in stage 4, in its own fenced block — see the "Forward to the ticket owner" block above>
   ```
...
**Approval gate — conditional on `require_plan_approval` (the plan-approval wait is optional per repo).** Read the resolved setting (`require_plan_approval`, from the step-6 snapshot / live via `settings.sh get`) and whether a Risk disclaimer was assembled above. Then branch:

- **Gate ON** — `require_plan_approval` is `true` (the default, whether unset-defaulting-true or explicit) — the normal, unchanged path: set `expected_next_action: "user-approval"` in STATE.json (the Stop hook allows the yield), present the plan summary (with the Critique section visible AND, if assembled above, the Risk disclaimer block) and **STOP**. Wait for explicit user approval (keywords: `approved`, `looks good`, `continue`, `proceed`, `yes`, `go ahead`).
- **Gate OFF but high-risk** — `require_plan_approval === false` AND a Risk disclaimer was assembled — the **safety override**: behave exactly like Gate ON (set `expected_next_action: "user-approval"`, present plan + disclaimer, STOP, wait). A disabled gate never skips the disclaimer stop. Log `{ phase: "define-approval", mode: "gate-off-high-risk-stop", triggers: [...], at }` to `state.history`. (No opt-out ask here — the gate is already off.)
- **Gate OFF and not high-risk** — `require_plan_approval === false` AND no disclaimer fired — **auto-approve, no wait.** Do NOT set `user-approval` and do NOT STOP. Emit the one-line informational banner (`▶ auto-task: <title> — Phase 1 (Define) — plan auto-approved (require_plan_approval=false); full plan at .auto-task/<branch>/PLAN.md`) as part of continuing (this is presentation woven into the auto-continue, not a yield), then take the on-approval transition below with `mode: "auto"`. The Clarifications/Decision-watchlist content still lives in PLAN.md for the audit trail even though it is not presented for a wait.

The `## Clarifications` surfacing and the **Decision watchlist** rendering below apply only on the two paths that actually present the plan for a wait (Gate ON, or Gate OFF + high-risk); on the auto-approve path they are recorded in PLAN.md but not surfaced for approval.
...
**One-time opt-out ask (only on a *manual* approval, when the gate was never explicitly decided).** After the user approves manually (Gate ON, or Gate OFF + high-risk path), and ONLY when `settings.sh present require_plan_approval` is `false` (the setting was never explicitly set at either scope — so a repo whose gate is unset-defaulting-true), ask ONCE — via a single `AskUserQuestion` — whether to disable the plan-approval gate for future runs in this repo. **Ordering:** issue this ask while `expected_next_action` is still `"user-approval"` (and `approved` still `false`), BEFORE writing the `approved: true`/`auto-continue` transition. The ask itself is an `AskUserQuestion` *tool call*, not a turn-end, so the Stop hook does not fire during it — but keeping the state at `user-approval`/`approved:false` until the opt-out choice is recorded means any incidental turn-end around the ask is allowed by the hook (whereas ending the turn after an `approved:true`/`auto-continue` write would be blocked), and it keeps the approval semantically un-finalized until the opt-out is handled. Do the persistence, then take the on-approval transition below. Wording (state the carve-out so it is not misleading right after a high-risk approval): *"Disable the plan-approval gate for future auto-task runs in this repo? Disabling skips routine plan approvals and runs unattended from intake — but high-risk runs (irreversible / HEAVY-tier / auth-payments) still pause, and clarifying questions still surface."* Options: **"Keep the gate (Recommended)"** / **"Disable it — auto-approve future plans"**. **Persist BOTH branches explicitly** so it never re-fires: keep → `settings.sh set require_plan_approval true`; disable → `settings.sh set require_plan_approval false` (project scope). **Fail-safe:** if the ask is dismissed / unanswered / `AskUserQuestion` is unavailable (headless), persist nothing and leave the gate ON — never silently disable a safety gate on a non-answer (it may simply re-ask on the next first-approval; that is acceptable). Log `{ phase: "define-approval-optout", decision: "keep|disable|dismissed", at }` to `state.history`. On the auto-approve path (Gate OFF + not high-risk) this ask does NOT run — the gate is already explicitly off, so `present` is already `true`.

On approval (manual approval typed, OR auto-approved on the Gate-OFF-not-high-risk path): write `approved: true`, `phase: "execute"`, AND `expected_next_action: "auto-continue"` to state, log `{ phase: "define-approval", mode: "manual|auto", at }` to `state.history`, then advance to Phase 2. From this point on, the Stop hook will block any attempt to end the turn until you reach a legitimate yield point or `phase: "done"`. **Do not commit on approval** — `.auto-task/<branch>/PLAN.md` stays out of git, and per the single-commit rule below, no code commit happens until Phase 5.
...
12. Write `pr_url` to state, then **decide the terminal transition**. Two post-PR phases can run, **in order: Phase 6 (bot-comment review) → Phase 7 (preview verification)**. Both require that a push actually happened (the user chose push/push-and-PR at step 8, not "hold"). Evaluate in this order:
    - **Push held / no PR opened** → no post-PR phase applies. Set `bot_review = { status: "skipped-no-push", at }` and `preview = { status: "skipped-no-push", verdict: null, at }`, `phase: "done"`, `expected_next_action: null`. Report the PR URL (if any) + a one-line diagram note + a one-line pointer that local context + artifacts live at `.auto-task/<branch>/`.
    - **Bot-comment review applicable** — a push happened, a PR was opened (`pr_url` set), AND `bot_review_autofix === true` (opt-in; default `false`) → set `phase: "bot-review"`, `expected_next_action: "auto-continue"`, advance to **Phase 6**. Do NOT report a final summary yet. Phase 6, when it completes, itself decides whether Phase 7 (preview) follows or the run is terminal.
    - **Else preview verification applicable** (bot-review not applicable) — a push happened AND preview applies per the **Phase 7 gate** (either explicit `has_preview_deployment === true`, OR **auto-learn eligible**: `has_preview_deployment` is unset AND `preview_autodetect !== false` AND a PR was opened) → set `phase: "preview"`, `expected_next_action: "auto-continue"`, advance to **Phase 7**. Do NOT report a final summary yet — Phase 7 owns the terminal state.
    - **Else** (neither applies — e.g. `has_preview_deployment` explicitly `false`, or `preview_autodetect: false` with no explicit opt-in) → set `bot_review = { status: "skipped-disabled", at }` and `preview = { status: "skipped-disabled", verdict: null, at }`, `phase: "done"`, `expected_next_action: null`. Report the PR URL + diagram note + local-artifacts pointer.
...
- **`expected_next_action` is mandatory and mechanically enforced.** Every state write that occurs after `approved: true` MUST include an `expected_next_action` value. The Stop hook reads this field on every turn-end and blocks the model from yielding when the value is `"auto-continue"`. The only legitimate user-* values are `"user-approval"` (Phase 1 plan presentation — when the gate is on OR the run is high-risk; the one-time gate opt-out ask; Loop-rule surface; destructive-action confirmation) and `"user-push-prompt"` (the single Phase 5 push/PR/hold ask). When `require_plan_approval` is off and the run is not high-risk there is no plan-presentation yield at all — the plan auto-approves on `"auto-continue"`. Setting a user-* value when no user gate is actually pending is a contract violation analogous to flipping a gate flag without running the gate. The Stop hook is the antidote to sub-skill output looking completion-shaped; do not work around it.
