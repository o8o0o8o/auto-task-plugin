...
| `visual_assets_enabled` | `false` | Opt-in master switch for **visual-proof embedding** (before/after screenshots uploaded to **Cloudinary** and embedded inline in the PR — see Phase 5 "Visual changes"). **Off by default**, per-project; auto-task **asks once per repo** (Phase-1 consent, only when the run has UI/visual scope) before it will upload images anywhere. When off, verification still runs locally; the PR just gets local-artifact + preview-link notes instead of embedded images. Enabling it uploads screenshots to a **public Cloudinary account** via an unsigned preset (public bytes; renders inline for public *and* private projects via GitHub's Camo proxy). Requires `cloudinary_cloud_name` + `cloudinary_upload_preset` to be set — until both are, embedding stays inert (the PR gets the local-artifact note). Settable per-project or globally (see "Two scopes"). |
| `cloudinary_cloud_name` | `""` | Your Cloudinary **cloud name** (shown on the Cloudinary dashboard). Not a secret — it appears in every delivery URL. Empty → visual embedding is inert (falls back to the local-artifact note even when `visual_assets_enabled` is true). |
| `cloudinary_upload_preset` | `""` | An **unsigned** Cloudinary upload preset (Dashboard → Settings → Upload → Upload presets → Signing Mode: Unsigned). Not a secret — unsigned upload needs no API key/secret. **Recommend restricting the preset** (allowed formats, max size, a fixed folder, moderation) since an unsigned preset is world-writable: anyone who learns the cloud name + preset can upload to your account and consume its free-tier quota. Empty → embedding is inert. |
...
4. **Ask exactly once** via a single `AskUserQuestion` (Phase-1 surface; set `expected_next_action: "user-approval"`): *"Embed before/after screenshots in PRs for visual changes? auto-task would upload UI screenshots to a **public Cloudinary account** via an unsigned preset and embed them inline (renders for public and private projects; the image bytes are public). Requires `cloudinary_cloud_name` + `cloudinary_upload_preset` in settings. Off by default; change anytime."* Options: **"Enable — embed screenshots in PRs"** / **"No thanks — don't ask again for this repo"**. Surface with the run banner.
...
6. **Config honesty (on enable).** If the user enabled it, check `bash "$settings_sh" get cloudinary_cloud_name` and `... get cloudinary_upload_preset`. If **either is empty**, keep `visual_assets_enabled: true` but surface a one-line note that embedding stays inert until both are set, naming the exact commands: `bash "$settings_sh" set cloudinary_cloud_name <name>` and `bash "$settings_sh" set cloudinary_upload_preset <unsigned-preset>` (create an **Unsigned** preset in Cloudinary → Settings → Upload → Upload presets). Do NOT try to collect the two values through `AskUserQuestion` (free-text is unreliable there). This keeps the opt-in honest: the consent never promises embedding the current config can't deliver.
