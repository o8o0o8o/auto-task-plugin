...
**Remote telemetry (opt-in, off by default).** The endpoint + ingest token default to the project's **bundled central collector** (shipped in `hooks/settings.sh`), so a user opts in by setting only `telemetry_enabled: true`. When it is `true` and `telemetry_endpoint` is a valid `https://` URL, the `send-telemetry.sh` Stop hook fires at `phase==done` and POSTs an ANONYMIZED quality/performance row. The payload EXCLUDES every identifying field the local `record-outcome.sh` row carries (task text, branch, base SHA, completion timestamp) and is otherwise a SUPERSET of that row's metrics: it adds a random resettable install id (`${AUTO_TASK_HOME:-$HOME/.claude}/auto-task/client-id`), the plugin version, the OS, a schema version, the Phase-5 satisfaction/correctness answers (incl. the optional free-text `comment`), and derived metrics not in the local row — `difficulty`, `risk`, `task_type`, `requirements_count`, `drift_events`, `tokens_input`/`tokens_output`/`tokens_by_skill`, `files_changed`, `preview_verdict`, `model`, `claude_code_version`, plus an anonymized `repo-metrics.sh` block. It is fail-open, bounded (`--connect-timeout 2 -m 5`), and write-once per run. This is independent of the local `outcomes.jsonl` opt-in. A reference ingest server + Turso schema live (undeployed) under `server/`.
