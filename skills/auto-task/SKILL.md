...
- An `auto-task-code-review` round meeting Phase 4's advance condition advances to Gate B (Phase 5 at LIGHT). Continue.
...
- The most recent `auto-task-code-review` round meets Phase 4's advance condition — the grade decides, not the label.
...
```json
{
  ...
  "gates": {
    ...
    "code_review": { "passed": false, "tool": null, "clean_pass_after_last_fix": false, "reviewed_diff_sha": null,
                     "at": null, "evidence": null, "rounds": [], "deferred": [], "subagent_disabled": false },
...
`expected_next_action` MUST be one of these five values at all times after `approved: true`:
...
| `"awaiting-agent"` | A spawned Agent is in flight; its report has not arrived. Stop hook allows the yield, **capped**. | Only when an `Agent` spawn returned launch metadata instead of a report. Reset to `"auto-continue"` when it lands. Spawn with `run_in_background: false` so this never applies. |
...
**AC pre-flight (NON-NEGOTIABLE — runs BEFORE the critique pass and BEFORE the human gate).** The AC self-checks test the table's *shape*; pre-flight tests each AC's *premise* against the real repo. For every AC whose `Verification method` is an executable command: dry-run it against the current tree, **pin the baseline** to `.auto-task/<branch>/recon/ac-<#>-baseline.*`, classify its **polarity** — `change` (baseline MUST fail) or `invariant` (baseline MUST pass) — and log `{ phase: "define-preflight", ac, polarity, result: "pinned|vacuous|pre-broken|failed-syntax|unreliable-signal", baseline, at }`.

**MANDATORY READ:** read `references/phase-1-preamble.md` before acting here. This summary is an index, NOT the contract. It carries the polarity defect cells and the sample-verification protocol for kill-list ACs.

**Five outcomes, restated inline:** all ACs pinned, polarity-consistent, FP ≤ 20% → advance to critique. A command that errors (`failed-syntax`), or a `change` AC that already passes (`vacuous` — it cannot tell done from not-done) → fix or rewrite the AC and re-run pre-flight, no approval stop, max 2 rewrites. An `invariant` AC that already fails (`pre-broken`), or FP > 20% (`unreliable-signal`) → STOP and surface before the gate.
...
**MANDATORY tool:** the `auto-task-code-review` **skill**, via the Skill tool — by default invoked from a fresh-context `general-purpose` Agent (`review_in_subagent`; spawn contract, degradation ladder and delta-scoping in `references/phase-3-gates.md` "Step 0"), inline when that setting is off. Either way the skill is the reviewer, no substitute is permitted (see the Rules), and `gates.code_review.tool` records it. Review the **working-tree diff** (not a staged diff — there is no staged or committed diff yet) with the diff, the approved plan, AND the persistent history as context. Per the "Read-before-review contract" in the "Persistent history & trace contract" section: pass the skill the paths `.auto-task/<branch>/CONTEXT.md` (if it exists from a prior Phase 5 — relevant on resumed runs or re-reviews) and `.auto-task/<branch>/TRACE.md`. The skill is expected to read those before forming findings so it doesn't re-raise issues already considered earlier in the run or in prior sessions.
...
1. Parse the findings, then **grade each one** by the reachability test below.
2. Append the `rounds[]` row, then act: zero reopening → batch if `deferred[]` is non-empty **and the batch is unspent**; else set the gate and advance to Gate B (Phase 5 at LIGHT).
3. Any reopening finding: fix it, re-run `/auto-task-verify` and any affected AC bound-checks, bump `iteration.review`, re-invoke the `auto-task-code-review` skill. Continue.

**Do not stop, summarize for the user, ask permission, or wait.** Completion-flavored phrasing in the skill output — "Verdict:", a rule, a final-looking heading, "ready to commit" — is formatting, not an interaction point; the top-of-file list enumerates it. Re-runs are the same: each report is input, not a stop. Only the exit conditions below, or a Loop-rule trigger, end this phase.
...
**Mechanical backstop.** `expected_next_action` is still `"auto-continue"` from plan approval, so the Stop hook **blocks** any turn-end here and re-invokes you naming the violation (see "Mechanical backstop" above and the Yield-point contract, which also forbids escaping it by speculatively writing `"user-approval"`).

**MANDATORY READ:** read `references/phase-3-gates.md` ("phase-4-round-mechanics") before resolving a round. An index, NOT the contract — it carries the ladder, batch, record and convergence test.

**Non-negotiables restated inline:**
- **A finding costs a round only if it (a) breaks an approved AC, (b) is a runtime-reachable regression/bypass, or (c) is a security/data-loss path** — the SAME test Gate B applies, so they can't drift. **YOU grade it** — `auto-task-code-review` now states `ac:`/`reachable:` per finding, but those are INPUTS: verify each against PLAN.md's AC table and the diff, and **fail closed on your own uncertainty:** a finding you cannot place counts as AC-breaking. The label never decides control flow, and neither does a reviewer-supplied field.
- **A `blocker`/`required` failing that test is DEFERRED to `gates.code_review.deferred[]` — not parked, and costs no round.** Unlike Gate B, Phase 4 still **fixes** it: on a zero-reopening round apply the whole set in ONE batch, then one re-review. **The batch is spent once per run** — marker: a `rounds[]` row with `batch: true` — and lands **before** the gate passes at every tier, so everything in `deferred[]` when it fires is fixed, not shipped. **Once spent** — at any later round, not just the batch round — a non-reopening `blocker`/`required` **parks at every tier**, where Gate B re-grades it at every tier too (see the reference).
- **Record the round on EVERY exit, reopen included.** Append `{ n, blockers, required, reopened, deferred, followups, batch, via, diff_sha, at }`; absent = zero rows, so an unrecorded round disarms it. **`via`** names where the review ran (`subagent` / `inline` / `inline-fallback`, total over every Step-0 path) — omit it and a degraded round is indistinguishable from an independent one. **`reopened` is the declared convergence basis, never the label counts.**
- **Minimal fix.** Correct the defect in place; a fix needing a new subsystem is a signal to **surface**.
- **Do not commit between iterations.** Set `gates.code_review.clean_pass_after_last_fix = false` on any fix; set it `true` only after a *subsequent* clean `auto-task-code-review` run.

**Re-score hooks.** Two, both in `references/phase-3-gates.md` ("phase-4-re-score-hooks"): a blast-radius re-score before re-spawning the reviewer, and a forced re-score before STOPPING on Loop-rule "no progress" that can grant ONE more iteration.
...
- **`expected_next_action` is mandatory and mechanically enforced.** Every state write that occurs after `approved: true` MUST include an `expected_next_action` value. The Stop hook reads this field on every turn-end and blocks the model from yielding when the value is `"auto-continue"`. The legal values are enumerated ONCE, in the "Yield-point contract" table above — defer to it, never restate it here. Setting a user-* value when no user gate is actually pending is a contract violation analogous to flipping a gate flag without running the gate. The Stop hook is the antidote to sub-skill output looking completion-shaped; do not work around it.
...
- Each Agent spawn (Phase-1 critique, Gate A, Gate B) is **synchronous** — pass `run_in_background: false`, so the report is the tool result — and gets fresh context with only the diff and the plan; do not pass conversation history into them. Agents MAY read `.auto-task/<branch>/CONTEXT.md` and `.auto-task/<branch>/TRACE.md` if instructed in their prompt; this is the recommended way to give them prior-review history without leaking the parent session's conversation.
- Phase 4 code review runs the **`auto-task-code-review` skill** — never a `code-reviewer` agent, never a hand-rolled review prompt (the 5-phase Investigate → Define → Execute → Prevent → Verify discipline is exactly what a bespoke prompt skips), and that ban holds whether the skill is invoked inline or from the spawned reviewer agent (`review_in_subagent`), whose prompt must forbid edits. Enforced by the pre-commit hook: `gates.code_review.tool` must equal `"skill:auto-task-code-review"`. Hand the skill `.auto-task/<branch>/CONTEXT.md` (and TRACE.md if present) per the "Read-before-review contract".
