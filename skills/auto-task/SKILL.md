...
```json
{
  ...
  "preview": {
    ...
    "polls": 0,
    "learned": "<{ value: true|false, at } when auto-learn persisted has_preview_deployment this run; null otherwise>",
...
| Key | Default | Meaning |
|---|---|---|
| `has_preview_deployment` | `false` | Whether this project has a preview deployment (drives whether Phase 7 preview verification runs). **Three-state via presence:** *unset* (never decided) → Phase 7 **auto-learns** on the first post-PR run: it detects whether a deployment exists and **persists the answer here** (`true` if found, `false` if not) so later runs are deterministic; explicit *`true`* → verify every run (no re-learn); explicit *`false`* → skip every run. An explicit value is never overwritten by auto-learn. |
| `preview_autodetect` | `true` | Gates the one-time **auto-learn** above: when `has_preview_deployment` is unset and a PR is opened, poll the PR's comments/body for a deployment URL (Vercel/Netlify/Cloudflare/… bot comment) via `pr-deploy-url.sh` and persist the result — zero config. Set `false` to disable auto-learn entirely (an unset `has_preview_deployment` then just means "no preview", and nothing is persisted). |
...
- **Auto-learn (first undecided run):** `has_preview_deployment` is **unset** (never explicitly decided — `settings.sh present has_preview_deployment` is `false`) AND `preview_autodetect !== false` (built-in default `true`) AND a PR was opened (`pr_url` set). In this path the pipeline detects whether a preview deployment exists (step 1) and then **persists the answer** to `has_preview_deployment` (step 1.5) so subsequent runs are deterministic: `true` if a deployment is found (check every time thereafter), `false` if none is found within the bound (don't check thereafter).
...
**1. Resolve the preview URL (bound to the pushed commit).** `HEAD_SHA = git rev-parse HEAD`.
   ...
   4. **No URL by `timeout_min`.** If NONE of the sources yields a reachable URL in the bound: in the **explicit** case (`has_preview_deployment` true) record `preview.status = "pending"` and surface (`user-approval`) to resume later. In the **auto-learn** case (`has_preview_deployment` was unset), do NOT nag: persist the learning `settings.sh set has_preview_deployment false` (per step 1.5 — this repo has no reachable preview, so don't check next time), record `preview = { status: "skipped-learned-none", learned: { value: false, at }, verdict: null, at }`, `phase: "done"`, `expected_next_action: null`, and finish with the normal final summary. A repo without a deploy comment must feel exactly like a repo with preview disabled — no failure, no stall, no extra prompt. Never emit FAIL for a missing preview (no preview ≠ a regression).
   5. **Persist the auto-learn result (only when `has_preview_deployment` was unset).** This closes the "detect once, then remember" loop:
      ...
      - **NO deployment found** by `timeout_min` (step 1.4) → `settings.sh set has_preview_deployment false` and record `preview.learned = { value: false, at }`. Future runs then hit the explicit-`false` branch and skip.
      - Locate `settings.sh` via the three-probe pattern; write the **project** scope (default). **Fail-open:** if `settings.sh set` errors, log `{ phase: "phase-7-preview-autolearn", learned: <bool>, result: "persist-failed", at }` to `state.history` and continue — a failed persist never blocks the run (next run simply re-learns). Log the success case as `{ phase: "phase-7-preview-autolearn", learned: <bool>, at }`. In the **explicit** cases (`has_preview_deployment` was set), skip this step entirely — never overwrite a user's explicit choice. (Edge, noted in the plan: a very slow deploy bot can cause a one-time false-negative `false`; the user overrides by setting `has_preview_deployment: true`.)
...
**4. Record + report.**
   ...
   - Append a TRACE entry: `operation: auto-task:phase-7-preview`, `outcome: <pass|fail|inconclusive|pending>`, summary covers the URL, verdict, any surfaced reason, and (when auto-learn ran) the persisted `has_preview_deployment` value.
...
**No preview infra?** With `preview_autodetect` at its `true` default, Phase 7 auto-learn is *attempted* on the first undecided run whenever a PR is opened, but a repo whose PRs get no deploy comment resolves no URL, so Phase 7 persists `has_preview_deployment: false`, records `skipped-learned-none`, and ends exactly as if preview were off — no failure, no stall, no extra prompt. From then on the explicit-`false` branch skips instantly with no polling. A repo that opens no PR (e.g. this plugin lands directly on `main`) never enters Phase 6 or Phase 7 at all. To opt out of auto-learn entirely, set `preview_autodetect: false` (or set `has_preview_deployment` explicitly either way).
