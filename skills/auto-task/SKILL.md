...
```json
{
  ...
  "preview": {
    "status": "skipped-disabled|skipped-no-push|skipped-no-url|awaiting|pending|verified|failed|inconclusive",
...
| Key | Default | Meaning |
|---|---|---|
| `has_preview_deployment` | `false` | Explicitly declare this project has a preview deployment (uses `gh` deployment API + `preview_url`). Independent of `preview_autodetect` below. |
| `preview_autodetect` | `true` | When a PR is opened, poll the PR's comments/body for a deployment URL (Vercel/Netlify/Cloudflare/… bot comment) and run **Phase 6** against it — zero config. Set `false` to disable the default preview auto-detection. No URL found in the bound ⇒ Phase 6 skips gracefully (no failure, no stall). |
...
12. Write `pr_url` to state, then **decide the terminal transition**:
    - **Preview verification applicable** — a push actually happened (the user chose push/push-and-PR at step 8, not "hold") AND EITHER `has_preview_deployment === true` (explicit) OR (`preview_autodetect !== false` AND a PR was opened, i.e. `pr_url` is set). The second clause is the **default, zero-config** path: with no preview settings, if a PR exists the pipeline will poll the PR for a deployment-URL comment (see Phase 6). When applicable, set `phase: "preview"`, `expected_next_action: "auto-continue"`, and advance to **Phase 6**. Do NOT report a final summary yet — Phase 6 owns the terminal state.
...
The last verification rung: after the change has been pushed, confirm it actually works on the **deployed preview**, then record a final judgment. This phase runs when a push happened in Phase 5 (the user did not "hold") AND either preview mode applies — otherwise Phase 5 step 12 already routed to `done`:

- **Explicit:** `state.settings.resolved.has_preview_deployment === true`, OR
- **Default auto-detect:** `preview_autodetect !== false` (the built-in default is `true`) AND a PR was opened (`pr_url` set). In this zero-config path the preview URL comes from the PR's own deploy comment (step 1.3 below), so preview verification works on any repo whose PRs get a Vercel/Netlify/etc. deploy comment, with no settings. If no such URL appears within the bound, Phase 6 skips gracefully (see step 1.4) — it never turns a missing deploy into a failure or a stall.
...
**No preview infra?** With `preview_autodetect` at its `true` default, Phase 6 is *attempted* whenever a PR is opened, but a repo whose PRs get no deploy comment resolves no URL and Phase 6 records `skipped-no-url` and ends exactly as if preview were off — no failure, no stall, no extra prompt. A repo that opens no PR (e.g. this plugin lands directly on `main`) never enters Phase 6 at all. To opt out of the auto-detection entirely, set `preview_autodetect: false`.
