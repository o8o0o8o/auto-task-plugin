...
**Exception 1 — the handover main-sync merge commit.** Phase 5 re-syncs the branch with `origin/<default>` right before the PR (see Phase 5 "pre-commit main-sync"). When main advanced during the run, `git merge --no-edit origin/<default>` produces a **merge commit** (and, on a resolved conflict, its `git commit --no-edit` finalize) on top of the single authored commit. That merge commit is **integration, not authored work** — it carries no new logic of this run — so it does not violate this rule. The `MERGE_HEAD` staleness exemption in `enforce-gates.sh` exists so this merge commit is not mistaken for an un-reviewed authored edit.
