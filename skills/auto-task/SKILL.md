...
7. **Telemetry consent (once per repo — NEW runs only).** Remote telemetry is opt-in and the destination is bundled (see "User settings"), so the *only* thing a user decides is whether to share. Ask that **once per repo**, never silently, never repeatedly:
   - Run `bash "$settings_sh" present telemetry_enabled`. If it returns `true`, the user has ALREADY decided (enabled or declined) at the project or global scope — do **not** ask; skip this step. If `false` (never asked in this repo), and this is a NEW run (skip entirely on resume), ask exactly once.
   - Present a single `AskUserQuestion` (part of the Phase-1 human surface; `approved` is still `false`, so the Stop hook allows it) BEFORE the clarifying questions: *"Share anonymous auto-task telemetry from this repo? Quality/performance metrics only — no code, task text, branch, paths, or identifiers leave your machine (see README → Remote telemetry). You can change this anytime in settings."* Options: **"Enable — share anonymous metrics"** / **"No thanks — don't ask again for this repo"**.
   - Record the choice IMMEDIATELY (before proceeding), project-scoped, so it is remembered and never re-asked: on enable → `bash "$settings_sh" set telemetry_enabled true`; on decline → `bash "$settings_sh" set telemetry_enabled false` (writing `false` is a real *decision* — `present` now returns `true`, so the prompt won't fire again). Refresh the `state.settings` snapshot afterward. Log `{ phase: "define-telemetry-consent", decision: "enabled|declined", scope: "project", at: "ISO-8601" }` to `state.history`.
   - Fail-open: if `settings.sh` can't be located or `AskUserQuestion` is unavailable (headless), skip the prompt and proceed with telemetry OFF (the default) — never block a run on consent, and never enable without an explicit answer. A global explicit `telemetry_enabled` (either value) also counts as "decided" and suppresses the per-repo prompt.
