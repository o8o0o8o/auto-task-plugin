...
- **Success:** Phase 5 completes (commit landed + PR open OR explicit user choice to hold the push at the final handover prompt) — and the post-PR phases run in order: **Phase 6 (bot-comment review, opt-in)** conservatively applies any actionable Cursor/GitHub-bot PR-comment fixes, then **Phase 7 (preview verification, when enabled/auto-learned)** records a final verdict (PASS/FAIL/INCONCLUSIVE), then **Phase 8 (external change application, when the run declared external actions)** applies + verifies the external change and only then reaches terminal `done` — a task that needs an external change is NOT `done` until Phase 8 applies it. A preview `handoff`/timeout (`pending`), a Phase-6 poll/surface, and a Phase-8 apply-prompt / `auto`-run settle-poll / `awaiting-external` handoff are legitimate post-PR surfaces per the yield-point table (work still owed), not mid-pipeline stalls.
...
There are NO other legitimate stopping points between plan approval and Phase 5 — and within Phase 5 only the two documented *procedural* prompts (the docs-update ask in `ask` mode when a change is proposed, then push/PR), plus the conditional step-7b merge gate and any fix-loop budget ack. In particular:
...
**The only sub-skill/sub-agent return that's allowed to end your turn is `auto-task-commit` after a successful push and `gh pr create` (Phase 5 final).** The one further exception is the `auto-task-docs` staleness report in `ask` mode, which — and only when it is non-empty — is what the Phase-5 docs prompt shows the user (Phase 5 step 1b). Everything else feeds the next step.
...
```json
{
  "phase": "define|execute|self-verify|gate-a|review|gate-b|handover|bot-review|preview|external|done",
...
| `"user-approval"` | A legitimate human gate. Stop hook allows. | Phase 1 plan-presentation point (before user types `approved` / `proceed`). Loop-rule surface (when the Surfacing protocol fires). The Phase-5 docs-update ask, when `docs_update_mode` is `ask` AND the step actually proposes a change (see Phase 5 step 1b). |
...
| Phase 8 `applied-verified` / accepted `applied-unverified` / descope / `none` | `null` (and `phase: "done"`) — a completed application is terminal |
...
1b. **Docs update (optional — `docs_update_mode`).** After a run, the repo's user-facing docs may describe behavior the run just changed. This sub-step refreshes them **before staging**, so the docs land in the SAME single handover commit and pass the same gates as the code — no second commit, no new carve-out to the Single-commit rule. It runs here, ahead of the change diagram / artifacts / CONTEXT.md, precisely because those three are derived from the diff: docs edits must already be in the tree when they are generated.
   ...
   6. **Re-pass the gates (MANDATORY) — now that every docs path is visible to `git diff`.** The edits are new authored bytes added *after* the review went clean, so the review is now stale by exactly the mechanism `enforce-gates.sh` blocks on. Re-run `/auto-task-verify` (current tier scope), re-invoke the **`auto-task-code-review`** skill on the new diff and drive it to a clean pass, then refresh `gates.code_review.reviewed_diff_sha` (Phase 4's pinned-flags formula) and `clean_pass_after_last_fix`. On **STANDARD/HEAVY** tier also set `gates.gate_b.passed = false` and **re-run Gate B** against the docs diff, setting it back to `true` **only on a clean adversarial pass** (a Blocker/Required there loops back to fixing, exactly as in the main pipeline) — the hook binds a freshness sha only to `code_review`, so without the reset it would accept the Gate B earned against the pre-docs diff (the identical reasoning as the Phase-6 bot-fix carve-out). Leaving it `false` would block the handover commit. **Interrupt safety (REQUIRED):** log `{ phase: "handover-docs", result: "re-gating", files: [<the authorized docs paths>], at }` to `state.history` BEFORE clearing **either** flag, so a session that dies mid-re-gate is recoverable — see the recovery clause in step 1. Note this window can leave `gates.code_review.passed` false as well (a Blocker/Required in the docs re-review loops back to fixing, which clears it), so the marker covers both flags, not just `gate_b`. LIGHT skips Gate B by design.
...
7. **Integrate main (merge), then verify no leak.** Unless the sync was skipped-offline in step 5, bring the branch up to date with main so the PR merges cleanly:
   - **Merge:** `git merge --no-edit origin/<default>`.
     ...
     - **Conflict → hybrid resolution policy:**
       ...
       - **Mandatory re-check BEFORE finalizing the merge commit:** the conflict resolution edited the run's own reviewed code, so re-run `/auto-task-verify` **AND** re-invoke the `auto-task-code-review` skill on the post-merge diff, drive it to a clean pass, **refresh `gates.code_review.reviewed_diff_sha`** to `hash(git diff <base>)` of the resolved tree (per Phase 4's pinned-flags formula), and — on **STANDARD/HEAVY** tier — **reset `gates.gate_b.passed = false` and re-run Gate B** against the merged diff (mirroring the Phase-6 bot-fix carve-out; LIGHT skips Gate B by design). `/auto-task-verify` alone is not enough — it runs tests/types/lint, not the review loop, so authored resolution logic would otherwise never be re-reviewed. **Only after that clean re-review**, finalize with **`git commit --no-edit`** — this literal `git commit` re-enters `enforce-gates.sh`, whose staleness check **now applies during a merge too** (there is no MERGE_HEAD exemption): the refreshed `reviewed_diff_sha` is precisely what admits the merge commit, and a skipped re-review leaves a stale sha that is **mechanically BLOCKED**. Do NOT lean on `git merge --continue` slipping past the hook's commit regex.
...
12. Write `pr_url` to state, then **decide the terminal transition**. Every branch below that writes `phase: "done"` is subject to the **run-metrics terminal guard** (step 4) — no `done` without a `handover-metrics` history entry — and to the external-actions rule. Two post-PR phases can run, **in order: Phase 6 (bot-comment review) → Phase 7 (preview verification)**. Both require that a push actually happened (the user chose push/push-and-PR at step 8, not "hold"). Evaluate in this order:
    ...
    - **Else** (none applies — e.g. `has_preview_deployment` explicitly `false`, `bot_review_autofix` false, and no external actions) → set `bot_review = { status: "skipped-disabled", at }`, `preview = { status: "skipped-disabled", verdict: null, at }`, `external = { status: "none", at }`, `phase: "done"`, `expected_next_action: null`. Report the PR URL + diagram note + local-artifacts pointer.
...
**This phase MAY make additional authored commits — the single exception to "only Phase 5 commits" beyond the main-sync merge.** Each bot-fix commit is permitted BECAUSE it passes the FULL gate machinery (re-verify + re-`auto-task-code-review` to a clean pass + refreshed `reviewed_diff_sha`) before `enforce-gates.sh` will allow it — exactly like the Phase-5 conflict-resolution re-review carve-out. The invariant "every authored commit is gate-reviewed" is preserved; the mechanical hook is unchanged (it already gates every non-`done`-phase commit). See the Single-commit rule's bot-fix carve-out.
...
**2. Apply the external action(s) (code shipped).** Load the external-action manifest (the `external.actions[]` produced during implement + Phase-1 declaration).
   ...
   - **`ask` (default):** one `AskUserQuestion` for permission + credentials (set `expected_next_action: "user-push-prompt"` — this is the single allowed Phase-8 prompt, like the Phase-5 push). On yes → run. On decline → fall to the runbook (step 3).
